...
// ---------- 전역 상수 ---------- (컴파일 시에 값 결정)
constexpr int    UPDATE_N = 10;     // N 프레임마다 얼굴 박스 새로 탐색
constexpr double EAR_THRESH = 0.20; // EAR 임계값 (높을수록 예민해짐)
// -------------------------------
...
int main() {
  ...
  while (true) {
    ...
    if (hasFace) {
      ...
      dlib::full_object_detection landmarks = sp(dlibFrame, biggestFaceRect);
      ...
      // 얼굴 박스 (녹색)
      cv::rectangle(frame,
        cv::Point(biggestFaceRect.left(), biggestFaceRect.top()),
        cv::Point(biggestFaceRect.right(), biggestFaceRect.bottom()),
        cv::Scalar(0, 255, 0), 2);
      ...
      if (earAvg < EAR_THRESH) {
        cv::putText(frame, "Eye Closed!!!!",
          cv::Point(biggestFaceRect.left(), biggestFaceRect.top() - 10),
